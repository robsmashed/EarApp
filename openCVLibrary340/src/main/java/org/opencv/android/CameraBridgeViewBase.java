        ...
        if (modified != null) {
            try {
                ...
                if(rotation == Surface.ROTATION_0) {
                    if(mCameraIndex == 1) {
                        Core.transpose(modified, rotated);
                        Core.flip(rotated, rotated, -1);
                        //Core.rotate(modified, rotated, Core.ROTATE_90_COUNTERCLOCKWISE);
                    }
                    else {
                        Core.transpose(modified, rotated);
                        Core.flip(rotated, rotated, 1);
                        //Core.rotate(modified, rotated, Core.ROTATE_90_CLOCKWISE);
                    }
                } else if(rotation == Surface.ROTATION_270){
                    if(mCameraIndex == 1) {
                        rotated = modified;
                        Core.flip(rotated, rotated, 0);
                        //Core.rotate(modified, rotated, Core.ROTATE_180);
                    }
                    else {
                        rotated = modified;
                        Core.flip(rotated, rotated, -1);
                    ...
                    if(mCameraIndex == 1) {
                        ...
                        rotated = modified;
                        //Core.flip(rotated, rotated, 0);
                        //Core.rotate(rotated, rotated, Core.ROTATE_90_CLOCKWISE);
                    }
                }

                //Utils.matToBitmap(modified, mCacheBitmap);
                Utils.matToBitmap(rotated, mCacheBitmap);
            } catch(Exception e) {
